...
<!-- Generated by scripts/generate-article-pages.py. Do not edit this file by
     hand. Every build overwrites it. Edit content/articles/[slug].html instead. -->
<html lang="en">
<head>
  <meta charset="utf-8" />
...
<div id="siteFooter"></div>
  ...
  <script>
  /* Click the cover photo to see it larger. protect-images.js does not
     intercept plain clicks, so this needs no cooperation from it.
     Pictures inside figure.article-fig are left to the figure lightbox in
     partials.js, which adds the caption and a close button. When this
     overlay handled them too, one click opened both, this one on top, so
     closing it revealed the second copy underneath. */
  (function () {{
    ...
    document.addEventListener('click', function (e) {{
      var img = e.target.closest &&
                e.target.closest('.article-hero img');
      if (img) {{
    ...
    for entry in articles:
        for old_slug in entry.get('redirects') or []:
            ...
            with open(path, 'w', encoding='utf-8') as f:
                f.write(
                    '<!DOCTYPE html>\n'
                    '<!-- Generated by scripts/generate-article-pages.py from the\n'
                    '     "redirects" array in site-data.json. Do not edit this file\n'
                    '     by hand. Every build overwrites it. -->\n'
                    '<html lang="en">\n<head>\n'
                    '<meta charset="utf-8">\n'
                    '<script>\n'
