...
def build_page(entry, prev_link, next_link, all_photos=None,
    ...
    multi = len(media) > 1
    # "compare": true on a two-picture photo entry swaps the pager for a
    # before/after slider. Only sensible when both pictures share the same
    # size and framing (a clean frame and its labelled copy, say), because the
    # second is laid exactly over the first and revealed from the left edge.
    compare = (not is_video_entry and len(media) == 2
               and bool(entry.get("compare")))
    slides = []
    ...
    if compare:
        slides_script = (
            "      <script>\n"
            "        (function(){\n"
            "          var box = document.querySelector('.share-compare');\n"
            "          if (!box) return;\n"
            "          var link  = box.querySelector('.share-open');\n"
...
<!-- Generated by scripts/generate-share-pages.py. Do not edit this file by
     hand. Every build overwrites it. Edit site-data.json instead. -->
<html lang="en">
<head>
  <meta charset="utf-8" />
