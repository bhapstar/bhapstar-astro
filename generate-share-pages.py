...
def build_page(entry, prev_link, next_link, all_photos=None, global_start=None):
    ...
    if not is_video_entry and lb_all:
        ...
        lightbox_script = (
            ...
            "      async function slbShare(){ var m=media[i]; var page=m.page||location.href; var title=m.alt||document.title; if(navigator.share){ var f=await slbFile(m.src); try{ if(f && navigator.canShare && navigator.canShare({files:[f]})){ await navigator.share({files:[f], title:title, text:title, url:page}); return; } }catch(_){} try{ await navigator.share({title:title, text:title, url:page}); }catch(_){} } else if(navigator.clipboard && navigator.clipboard.writeText){ try{ await navigator.clipboard.writeText(page); slbFlash(); }catch(_){} } }\n"
